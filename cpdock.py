...
class statusWriter:
    ...
    def write(self, status, clearText=False, showEnd=True):
...
class dockWindow:
    def __init__(self):
        ...
        self.statusMessageTextBox = Text(self.statusMessageTextFrame, height=10,
                                         yscrollcommand=self.statusMessageScrollbar.set,
                                         background='dimgray', foreground='ghost white', insertbackground='ghost white',
                                         selectforeground='dimgray', selectbackground='ghost white')
        ...
        self.button1 = ttk.Button(self.buttonFrame, text="Delimiter to Separator", command=self.delimiterToSeparator,
                                  width=25)
        self.button1.grid(column=0, row=2, sticky=(N, S))
        self.button1.bind('<Return>', self.delimiterToSeparator)

        self.button2 = ttk.Button(self.buttonFrame, text="Delimiter to WrappedSep",
                                  command=self.delimiterToSeparatorWrapped, width=25)
    ...
    def pickFromStack(self, *args):
        ...
        if self.mainClipboard.getCurrentItem() is not None:
    ...
    def popClipboardStack(self, *args, cycle=False):
        ...
        if poppedItem is not None and len(poppedItem) > 0:
    ...
    def newlineToComma(self, *args):
        try:
            ...
            self.statusMessageTextBoxWriter.write(
                "Error in executing newlineToComma transform, stack probably empty...")
    ...
    def saveToFile(self, *args):
        try:
            toAdd = str(self.root.clipboard_get())
            filename = filedialog.asksaveasfilename(initialdir=INITIAL_FILE_SAVE_DIRECTORY,
                                                    title="Please select filename and location...")
